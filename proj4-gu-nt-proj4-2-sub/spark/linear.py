...
class LinearClassifier(Classifier):
  ...
  def backward(self, data, count):
    ...
    """
    back = softmax.map(lambda (x,(L,df)):np.dot(x.T, df))
    """
    ...
    """
    dLdA = back.reduce(lambda a,b:a+b) # replace it with your code
    dLdb = softmax.map(lambda (x,(L,df)):np.sum(df, axis=0))\
