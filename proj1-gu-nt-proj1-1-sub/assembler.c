...
int pass_one(FILE* input, FILE* output, SymbolTable* symtbl) {
    ...
    while (fgets(buf, BUF_SIZE, input)) {
        ...
        input_line++;
        skip_comment(buf);
        ptr = strtok(buf, IGNORE_CHARS);
        if (ptr!=NULL) {
            int p  = add_if_label(input_line,ptr,byte_offset,symtbl);
            if (p == 0) {
                strcpy(name, ptr);
                ptr = strtok(NULL, IGNORE_CHARS);
                while (ptr!=NULL) {
                    if (num_args == 3) {
                ...
                if (!extra_arg) {
                    int cnt = write_pass_one(output, name, args, num_args);
                    if (cnt==0) {
